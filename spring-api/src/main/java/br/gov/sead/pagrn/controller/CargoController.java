...
public class CargoController {
    ...
    public ResponseEntity<CargoDtoResponse> insert(@Valid @RequestBody CargoDtoRequest dto) {
        ...
        Cargo cargoSaved = service.create(cargo);
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<CargoDtoResponse> update(@Valid @RequestBody CargoDtoRequest dtoRequest, @PathVariable Long id) {
        try{
            Cargo cargo = modelMapper.map(dtoRequest, Cargo.class);
            Cargo cargoUpdated = service.update(id, cargo);
            CargoDtoResponse cargoDtoResponse = modelMapper.map(cargoUpdated, CargoDtoResponse.class);
            return new ResponseEntity<>(cargoDtoResponse,HttpStatus.OK);
