...
@SQLDelete(sql = "UPDATE aproveitamentos SET removed = CURRENT_DATE WHERE id=?")
@Getter
@Setter
@ToString
...
public class Aproveitamento extends Evento implements Serializable {
    ...
    @NotNull(message = ApiMessages.NIVEL_CARGO_NAO_PODE_SER_NULO)
    @ManyToOne
    ...
    public void atualizarVinculo(Vinculo vinculo) {
        vinculo.setDataPosse(this.dataPosse);
        vinculo.setDataInicioExercicio(this.dataInicialExercicio);
        vinculo.setNivelCargo(this.nivelCargo);
    ...
    public void preencher(NivelCargo nivelCargo, Setor setor) {
